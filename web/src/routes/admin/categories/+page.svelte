<script lang="ts">
  ...
  let dialogOpen = $state(false); let editItem = $state<any | null>(null); let deleteId = $state<number | null>(null)
  ...
  function openCreate() { editItem = null; name = ''; desc = ''; dialogOpen = true }
  function openEdit(cat: any) { editItem = cat; name = cat.name; desc = cat.description || ''; dialogOpen = true }

  async function handleSave() {
    if (!name.trim()) return; const tok = t(); if (!tok) return
    try {
      if (editItem) { const r = await fetch(`/api/categories/${editItem.id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${tok}` }, body: JSON.stringify({ name, description: desc || null }) }); const j = await r.json(); if (j.data) items = items.map((c: any) => c.id === editItem.id ? j.data : c) }
      else { const r = await fetch('/api/categories', { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${tok}` }, body: JSON.stringify({ name, description: desc || null }) }); const j = await r.json(); if (j.data) items = [...items, j.data] }
      dialogOpen = false
    } catch (e: any) { alert(e.message) }
  }

  async function handleDelete() { if (!deleteId) return; const tok = t(); if (!tok) return; try { await fetch(`/api/categories/${deleteId}`, { method: 'DELETE', headers: { Authorization: `Bearer ${tok}` } }); items = items.filter((c: any) => c.id !== deleteId) } catch (e: any) { alert(e.message) } finally { deleteId = null } }
...
<div class="flex flex-1 flex-col gap-4">
  <div class="flex flex-wrap items-end justify-between gap-2">
    <div><h2 class="text-2xl font-bold tracking-tight">Categories</h2><p class="text-muted-foreground">Organize content</p></div>
    <Button onclick={openCreate}><Plus class="mr-2 h-4 w-4" /> New Category</Button>
  </div>

  <div class="flex items-center justify-between">
    <div class="flex flex-1 flex-col-reverse items-start gap-y-2 sm:flex-row sm:items-center sm:space-x-2">
      <Input placeholder="Filter categories..." bind:value={search} class="h-8 w-[150px] lg:w-[250px]" oninput={() => page = 1} />
    </div>
  </div>
    ...
    <Table>
      <TableHeader>
        <TableRow class="group/row">
          <TableHead class="bg-background group-hover/row:bg-muted w-10"><Checkbox checked={allSelected} indeterminate={someSelected} onCheckedChange={toggleAll} aria-label="Select all" class="translate-y-0.5" /></TableHead>
          ...
          <TableHead class="bg-background group-hover/row:bg-muted group-data-[state=selected]/row:bg-muted"><span class="text-xs font-medium text-muted-foreground">Description</span></TableHead>
          <TableHead class="bg-background group-hover/row:bg-muted group-data-[state=selected]/row:bg-muted w-24"></TableHead>
        </TableRow>
      </TableHeader>
      <TableBody>
        {#if loading}
          <TableRow><TableCell colspan={5} class="h-24 text-center text-muted-foreground">Loading...</TableCell></TableRow>
        {:else if paged.length === 0}
          <TableRow><TableCell colspan={5} class="h-24 text-center text-muted-foreground">No categories.</TableCell></TableRow>
        {:else}
          {#each paged as cat (cat.id)}
            <TableRow data-state={rowSelection.has(cat.id) && 'selected'} class="group/row">
              <TableCell class="bg-background group-hover/row:bg-muted group-data-[state=selected]/row:bg-muted"><Checkbox checked={rowSelection.has(cat.id)} onCheckedChange={() => toggle(cat.id)} aria-label="Select row" class="translate-y-0.5" /></TableCell>
              <TableCell class="bg-background group-hover/row:bg-muted group-data-[state=selected]/row:bg-muted font-medium">{cat.name}</TableCell>
              <TableCell class="bg-background group-hover/row:bg-muted group-data-[state=selected]/row:bg-muted text-sm text-muted-foreground">/{cat.slug}</TableCell>
              <TableCell class="bg-background group-hover/row:bg-muted group-data-[state=selected]/row:bg-muted text-sm text-muted-foreground max-w-48 truncate">{cat.description || '—'}</TableCell>
              <TableCell class="bg-background group-hover/row:bg-muted group-data-[state=selected]/row:bg-muted">
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end" class="w-40">
                    ...
                    <DropdownMenuItem class="text-red-500 focus:text-red-500!" onclick={() => deleteId = cat.id}>
                      Delete
